...
export const CodeExporter: React.FC<CodeExporterProps> = ({ username }) => {
  const [copied, setCopied] = useState(false);

  const getScriptableCode = () => {
 ...
 * 8. In "Parameter", enter your GitHub username (e.g., '${username}').
 */

// --- USER CONFIGURATION ---
const DEFAULT_USERNAME = "${username}"; // Fallback username if parameter is empty
...
`;
  ...
  const handleCopy = async () => {
    try {
      await navigator.clipboard.writeText(getScriptableCode());
      setCopied(true);
  ...
  return (
    <div className="glass-panel animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      <div>
        <h2 style={{ fontSize: '1.25rem', fontWeight: 700, marginBottom: '6px', color: 'var(--accent-purple)' }}>
          iPhone Scriptable Setup
        </h2>
        <p style={{ fontSize: '0.875rem', color: 'var(--text-muted)' }}>
          Follow these simple steps to install the widget on your iPhone:
        </p>
      </div>

      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', fontSize: '0.875rem' }}>
        <div style={{ display: 'flex', gap: '10px' }}>
          <div style={{ background: 'var(--accent-blue)', color: 'var(--bg-color)', width: '20px', height: '20px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: '0.75rem', flexShrink: 0 }}>
            1
          </div>
          <div>
            Install the free <strong>Scriptable</strong> app from the iOS App Store.
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
          ...
          <div>
            Click the button below to copy the widget code to your clipboard.
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
          ...
          <div>
            Open Scriptable on your iPhone, tap <strong>+</strong>, paste the code, and name the script <strong>"GitHub Streak"</strong>.
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
          ...
          <div>
            Add a Scriptable widget to your Home Screen, tap it to edit, select <strong>"GitHub Streak"</strong>, and enter <strong>{username}</strong> in the Parameter field.
          </div>
        </div>
      </div>

      <button className="btn" onClick={handleCopy} style={{ marginTop: '8px' }}>
        {copied ? 'Copied to Clipboard! ✓' : 'Copy Widget Code'}
      </button>

      <div className="code-preview-container">
        <div className="code-header">
          <span className="code-title">github_streak_widget.js</span>
          <button className="copy-btn" onClick={handleCopy}>
            {copied ? 'Copied' : 'Copy'}
          </button>
        </div>
        <pre className="code-pre">
          <code>{getScriptableCode()}</code>
        </pre>
      </div>
    </div>
  );
